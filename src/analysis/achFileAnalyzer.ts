...
export function classifyTransaction(code: string): TransactionType {
	if (['22', '23', '32', '33'].includes(code)) {
		return 'Credit';
	}

	if (['27', '28', '37', '38'].includes(code)) {
		return 'Debit';
	}
...
function maskAccount(account: string): string {
	...
	return `${'*'.repeat(Math.max(6, trimmed.length - 4))}${trimmed.slice(-4)}`;
}
...
export function analyzeAchFile(lines: string[]): AchFileAnalysis {
	...
	for (let index = 0; index < lines.length; index++) {
		...
		if (recordType === '5') {
			currentBatch = {
				batchNumber: value(line, 88, 94),
				companyName: value(line, 5, 20),
				companyId: value(line, 41, 50),
				secCode: value(line, 51, 53),
				serviceClass: value(line, 2, 4),
				odfiIdentification: value(line, 80, 87),
				entryDescription: value(line, 54, 63),
				effectiveEntryDate: value(line, 70, 75),
				entries: [],
				addendaCount: 0,
				debitCount: 0,
				creditCount: 0,
			...
			lastEntry = {
				...
				transactionType,
				amountCents,
				receivingDfi: value(line, 4, 11),
				maskedAccountNumber: maskAccount(
					value(line, 13, 29)
				),
				individualId: value(line, 40, 54),
				individualName: value(line, 55, 76),
				addendaIndicator: value(line, 79, 79),
				addendaCount: 0,
				traceNumber: value(line, 80, 94)
			};

			currentBatch.entries.push(lastEntry);

			currentBatch.entryHash = hash(
				numberValue(line, 4, 11),
				currentBatch.entryHash
			);

			if (transactionType === 'Debit') {
				currentBatch.debitCount++;
				currentBatch.debitCents += amountCents;
			}

			if (transactionType === 'Credit') {
				currentBatch.creditCount++;
				currentBatch.creditCents += amountCents;
			}
	...
	const entries = batches.flatMap(
		batch => batch.entries
	);

	const debitCents = batches.reduce(
		(total, batch) => total + batch.debitCents,
		0
	);

	const creditCents = batches.reduce(
		(total, batch) => total + batch.creditCents,
		0
	);

	const entryHash = batches.reduce(
		(total, batch) => hash(batch.entryHash, total),
		0
	);

	const entryAddendaCount =
		entries.length + addendaCount;
